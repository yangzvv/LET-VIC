...
class VICTrackLidarPointCloud(PointCloud):
    ...
    def _render_helper(self,
        ...
        # 设置颜色为黑色，并增大点大小
        ax.scatter(points[0, :], points[1, :], c='black', s=marker_size * 5)  # s=marker_size*3 让点更大

        ax.set_xlim(x_lim[0], x_lim[1])
        ax.set_ylim(y_lim[0], y_lim[1])

    @classmethod
    def from_file(cls, file_name: str) -> 'VICTrackLidarPointCloud':
        """
        Loads LIDAR data from either binary numpy format (.bin) or PCD format (.pcd).
        For .bin files, data is stored as (x, y, z, intensity, ring index).
        For .pcd files, data is stored as (x, y, z, intensity).
        :param file_name: Path of the pointcloud file on disk.
        :return: LidarPointCloud instance (x, y, z, intensity).
        """
        if file_name.endswith('.bin'):
            # Load binary numpy data (x, y, z, intensity, ring index)
            ...
            # Convert open3d point cloud to numpy array (Nx3)
            points = np.asarray(pcd.points)
            
            # Assuming intensity is stored as a property, extract it if available
            # If intensity is not available, set it to a default value (e.g., 1)
            if len(pcd.colors) > 0:
                # In PCD format, colors are typically stored in RGB format, use it as intensity
                intensity = np.asarray(pcd.colors)[:, 0]  # Use the red channel as intensity
            else:
                # Default intensity if no colors are available
                intensity = np.ones(len(points))
            
            # Combine points (x, y, z) and intensity into a 4D array
            lidar_data = np.column_stack((points, intensity))
            
            return cls(lidar_data.T)
...
class BEVRender(BaseRender):
    ...
    def show_lidar_data(
        ...
        data_path, boxes, _ = nusc.get_sample_data(
            lidar_record, selected_anntokens=sample_record['anns'])
        VICTrackLidarPointCloud.from_file(data_path.replace('.bin', '.pcd')).render_height(
    ...
    def render_pred_box_data(self, agent_prediction_list):
        if agent_prediction_list is None:
            return
        for pred_agent in agent_prediction_list:
            ...
            if hasattr(pred_agent, 'pred_track_id') and pred_agent.pred_track_id is not None:  # this is true
                ...
                c = color_mapping[int(tr_id) % len(color_mapping)]
    ...
    def render_pred_traj(self, agent_prediction_list, top_k=3):
        ...
        for pred_agent in agent_prediction_list:
            ...
            if pred_agent.pred_traj is None:
                continue
