    ...
    for frame_id in tqdm(dict_frame_id2anno.keys()):
        ...
        for i in frame_data:
            ...
            if 'tracking_id' in i:
                ...
                tracking_name = i['tracking_name']

            sus_label_data["obj_id"] = tracking_id  # "-".join(instance_token.split('-')[0:-1])
            sus_label_data["obj_type"] = tracking_name  # instance_token.split('-')[-1] #"Car"
            sus_label_data["psr"] = {}
            sus_label_data["psr"]["position"] = {}
            sus_label_data["psr"]["position"]["x"] = v_label_lidar_3d_location[0]
            sus_label_data["psr"]["position"]["y"] = v_label_lidar_3d_location[1]
...
def gen_sus_label_from_univ2x_pkl(spd_dataset_path, vi, pkl_path, output_path):

    pkl_info = read_pkl(pkl_path)
    ...
    for samp in tqdm(pkl_info['infos']):
        frame_id = samp['token']
        ...
        list_sus_label_info = []
        for j in range(len(samp['gt_boxes'])):
            gt_boxes = samp['gt_boxes'][j]
            gt_inds = str(samp['gt_inds'][j])
            gt_names = str(samp['gt_names'][j])

            sus_label_data = {}
            # instance_token = i["instance_token"]

            sus_label_data["obj_id"] = gt_inds  # "-".join(instance_token.split('-')[0:-1])
            sus_label_data["obj_type"] = gt_names # instance_token.split('-')[-1] #"Car"
            sus_label_data["psr"] = {}
            sus_label_data["psr"]["position"] = {}
            sus_label_data["psr"]["position"]["x"] = float(gt_boxes[0])
            sus_label_data["psr"]["position"]["y"] = float(gt_boxes[1])
            sus_label_data["psr"]["position"]["z"] = float(gt_boxes[2])
            sus_label_data["psr"]["rotation"] = {}
            sus_label_data["psr"]["rotation"]["x"] = 0.0
            sus_label_data["psr"]["rotation"]["y"] = 0.0
            sus_label_data["psr"]["rotation"]["z"] = -float(gt_boxes[6]) - np.pi / 2
            sus_label_data["psr"]["scale"] = {}
            sus_label_data["psr"]["scale"]["x"] = float(gt_boxes[4])
            sus_label_data["psr"]["scale"]["y"] = float(gt_boxes[3])
            sus_label_data["psr"]["scale"]["z"] = float(gt_boxes[5])
            # print(sus_label_data)
            list_sus_label_info.append(sus_label_data)
        write_json(output_label_file_path, list_sus_label_info)
