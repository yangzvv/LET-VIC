...
def create_spd_infos_coop(root_path,
    ...
    for coop_data_info in tqdm(coop_data_infos):
        ...
        # info['lidar'] = {}
        # lidar_type = 'VEHICLE_TOP'
        # info['lidar'][lidar_type] = {}
        # info['lidar'][lidar_type]['data_path'] = data_info['pointcloud_path']
        ...
        # info['lidar'][lidar_type]['lidar2ego_rotation'] = calib_lidar2ego['transform']['rotation']
        # info['lidar'][lidar_type]['lidar2ego_translation'] = calib_lidar2ego['transform']['translation']
        ...
        # info['lidar'][lidar_type]['ego2global_rotation'] = calib_ego2global['rotation']
        # info['lidar'][lidar_type]['ego2global_translation'] = calib_ego2global['translation']
        ...
        calib_virtuallidar2global_path = osp.join(root_path, 'infrastructure-side',
        ...
        info['VehLidar2InfLidar_rotation'], info['VehLidar2InfLidar_translation'] = r,t  # r * x + t.T
        ...
        info['cams'][camera_type]['lidar2cam_translation'] = np.array(calib_lidar2cam['translation']).reshape(3)
        ...
        # yzw
        camera_type_inf = 'INFRASRUCTURE_CAM_FRONT'
        info['cams'][camera_type_inf] = {}
        info['cams'][camera_type_inf]['data_path'] = inf_data_info['image_path']

        calib_lidar2cam_path_inf = osp.join(root_path, 'infrastructure-side', inf_data_info['calib_virtuallidar_to_camera_path'])
        calib_lidar2cam_inf = load_json(calib_lidar2cam_path_inf)
        # veh_lidar2inf_cam_r = np.array(calib_lidar2cam_inf['rotation']) @ info['VehLidar2InfLidar_rotation']
        # veh_lidar2inf_cam_t = np.array(calib_lidar2cam_inf['rotation']) @ info['VehLidar2InfLidar_translation'].reshape(3, 1) + np.array(calib_lidar2cam_inf['translation'])
        veh_lidar2inf_cam_r, veh_lidar2inf_cam_t = mul_matrix(
            info['VehLidar2InfLidar_rotation'], info['VehLidar2InfLidar_translation'].reshape(3, 1), np.array(calib_lidar2cam_inf['rotation']), np.array(calib_lidar2cam_inf['translation'])
            )
        info['cams'][camera_type_inf]['lidar2cam_rotation'] = veh_lidar2inf_cam_r
        info['cams'][camera_type_inf]['lidar2cam_translation'] = veh_lidar2inf_cam_t.reshape(3)

        ## UniV2X TODO: check the correction
        inf_cam2veh_lidar_r = np.linalg.inv(veh_lidar2inf_cam_r)
        inf_cam2veh_lidar_t = - np.array(veh_lidar2inf_cam_t).reshape(1, 3) @ inf_cam2veh_lidar_r.T
        ## UniV2X TODO: check the matrix transformation with Quaternion
        info['cams'][camera_type_inf]['sensor2lidar_rotation'] = inf_cam2veh_lidar_r
        info['cams'][camera_type_inf]['sensor2lidar_translation'] = inf_cam2veh_lidar_t.reshape(3)

        ## UniV2X TODO: check the correction
        inf_cam2veh_ego_r, inf_cam2veh_ego_t = mul_matrix(inf_cam2veh_lidar_r, inf_cam2veh_lidar_t,
                                                            Quaternion(info['lidar2ego_rotation']).rotation_matrix, np.array(info['lidar2ego_translation']))
        ## UniV2X TODO: check the matrix transformation with Quaternion
        info['cams'][camera_type_inf]['sensor2ego_rotation'] = inf_cam2veh_ego_r
        info['cams'][camera_type_inf]['sensor2ego_translation'] = inf_cam2veh_ego_t.reshape(3) 

        calib_cam_intrinsic_path_inf = osp.join(root_path, 'infrastructure-side', inf_data_info['calib_camera_intrinsic_path'])
        calib_cam_intrinsic_inf = get_cam_intr(calib_cam_intrinsic_path_inf)
        info['cams'][camera_type_inf]['cam_intrinsic'] = calib_cam_intrinsic_inf
        ...
        gt_boxes = np.concatenate([locs, dims, -rots - np.pi / 2], axis=1)  # yzw top lidar 坐标系 x指向右侧，wlh相对于向右转90度
...
def cal_ego_velocity(data_infos,sample_info_mappings,lidar_ego_global_infos):
    ...
    ego_velocity = {}
    for data_info in tqdm(data_infos):
        if int(data_info["sequence_id"]) < 99:
            sample_token = data_info['frame_id']
...
def create_spd_infos(root_path,
    ...
    for data_info in tqdm(data_infos):
        # if int(data_info["sequence_id"]) < 99:
            ...
            gt_boxes = np.concatenate([locs, dims, -rots - np.pi / 2], axis=1)  # yzw
            names = np.array([b.name for b in boxes])
            instance_tokens = np.array([b.instance_token for b in boxes])
            instance_inds = np.array([b.track_id for b in boxes])
            box_tokens = np.array([b.token for b in boxes])
...
def _get_total_annotations_coop(root_path, data_infos, sample_info_mappings):
    ...
    for data_info in data_infos:
        ...
        annotation_path = osp.join(root_path, 'cooperative/label', f'{int(sample_token[1:]):06d}' + '.json')
        annotations = load_json(annotation_path)
        total_annotations[sample_token] = {}

        for annotation in annotations:
            # anno_token = annotation['token']
            # yzw Delay DATASET需要唯一指定
            anno_token = gen_token(annotation['token'], scene_token)
            annotation['token'] = anno_token
            annotation["instance_token"] = gen_token(annotation['track_id'], scene_token)  # yzw
...
def parse_args():
    ...
    parser.add_argument('--no-err-offset', action='store_true', default=False, help='Whether to offset calib error.')
...
if __name__ == "__main__":
    ...
    # import sys
    # sys.path.append("/home/yangwenxian/project/UniV2X")
    ...
    if v2x_side == 'cooperative':
        print("no-err-offset:", args.no_err_offset)
        ...
        total_annotations, sample_info_mappings, spd_infos = create_spd_infos_coop(data_root,
                                                                                   ...
                                                                                   can_bus_root_path,
                                                                                   info_prefix,
                                                                                   no_err_offset=args.no_err_offset,
