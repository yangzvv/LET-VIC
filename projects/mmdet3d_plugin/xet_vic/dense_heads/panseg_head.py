...
import mmcv, os
from ..modules import CrossLaneInteraction
...
class PansegformerHead(SegDETRHead):
    ...
    def __init__(
        ...
        self.save_lane_query = save_lane_query
        ...
        if self.is_cooperation:
            self.cross_lane_fusion = CrossLaneInteraction(pc_range=self.pc_range,
    ...
    def _get_coop_bev_embed(self, memory_src, memory_pos_src, lane_query, lane_query_pos, reference, start_idx=300):
        ...
        inf_lane_query = lane_query[0][start_idx:]
        inf_lane_query_pos = lane_query_pos[0][start_idx:]
        inf_reference = reference[0][start_idx:]

        locs = inf_reference.sigmoid().clone()
        ...
        for idx in range(inf_reference.shape[0]):
            w = int(locs[idx, 0])
            h = int(locs[idx, 1])
            ...
            for hh in range(max(0, h - pixel_len), min(self.bev_h - 1, h + pixel_len)):
                for ww in range(max(0, w - pixel_len), min(self.bev_w - 1, w + pixel_len)):
                    bev_embed_new[:, hh * self.bev_w + ww, :] =  bev_embed[:, hh * self.bev_w + ww, :] + self.bev_embed_linear(inf_lane_query[idx])
                    bev_pos_new[:, hh * self.bev_w + ww, :] =  bev_pos[:, hh * self.bev_w + ww, :] + self.bev_pos_linear(inf_lane_query_pos[idx])

        return bev_embed_new, bev_pos_new

    '''
    def _get_coop_bev_embed(self, bev_embed_src, inf_lane_query, inf_lane_query_pos, inf_reference):
        bev_embed = bev_embed_src
        inf_lane_query = inf_lane_query[0]
        inf_lane_query_pos = inf_lane_query_pos[0]
        inf_reference = inf_reference[0]
        ...
        for idx in range(inf_reference.shape[0]):
            ...
            for hh in range(max(0, h - pixel_len), min(self.bev_h - 1, h + pixel_len)):
                for ww in range(max(0, w - pixel_len), min(self.bev_w - 1, w + pixel_len)):
                    bev_embed[hh * self.bev_w + ww, :, :] =  bev_embed[hh * self.bev_w + ww, :, :] + self.bev_embed_linear(inf_lane_query[idx])

        return bev_embed
    '''

    @force_fp32(apply_to=('bev_embed', ))
    def forward(self, bev_embed, img_metas=None, veh2inf_rt=None):
        """Forward function.
        ...
        Returns:
            ...
            enc_outputs_coord (Tensor): The proposal generate from the \
                ...
                `None` would be returned.
        """
        ## UniV2X: cross lane fusion
        if self.is_cooperation:
            inf_lane_data_path = os.path.join(self.read_lane_query_file_root, img_metas[0]['sample_idx_inf'] +'.pkl')
            inf_lane_data = mmcv.load(inf_lane_data_path)
            for key in inf_lane_data.keys():
                inf_lane_data[key] = inf_lane_data[key].to(bev_embed)

        # if self.is_bev_aug:
        #     inf_query, inf_query_pos, inf_reference = self.cross_lane_fusion.get_inf2veh_query(
        #         inf_lane_data['outputs_classes'], inf_lane_data['query'], inf_lane_data['query_pos'], inf_lane_data['reference'],
        #         veh2inf_rt)
        #     bev_embed = self._get_coop_bev_embed(bev_embed, inf_query, inf_query_pos, inf_reference)
        ...
        if self.is_cooperation:
            outputs_classes, outputs_coords, query, query_pos, reference = self.cross_lane_fusion(
                inf_lane_data['outputs_classes'], inf_lane_data['outputs_coords'], inf_lane_data['query'], inf_lane_data['query_pos'], inf_lane_data['reference'],
    ...
    def forward_test(self,
        ...
        pred_seg_dict = self(pts_feats, img_metas=img_metas, veh2inf_rt=veh2inf_rt)

        ## UniV2X: inf_lane_query
        if self.save_lane_query:
            tensor_to_cpu = torch.zeros(1)
            memory, memory_mask, memory_pos, query, _, query_pos, hw_lvl = pred_seg_dict['args_tuple']
            save_path = os.path.join(self.save_lane_query_file_root, img_metas[0]['sample_idx'] +'.pkl')
            lane_save_data = {'outputs_classes': pred_seg_dict['outputs_classes'].to(tensor_to_cpu),
                                    'outputs_coords': pred_seg_dict['outputs_coords'].to(tensor_to_cpu),
                                    'query': query.to(tensor_to_cpu),
                                    'query_pos': query_pos.to(tensor_to_cpu),
                                    'reference': pred_seg_dict['reference'].to(tensor_to_cpu)}
            mmcv.dump(lane_save_data, save_path)
        ...
        with torch.no_grad():
            ...
            # import cv2
            # import os
            # save_root = './work_space/1111/1112_vis_lanefusion_debug_coop_with_fusion_epoch_4'
            # img_num = img_metas[0]['filename'][0].split('/')[-1].replace('.jpg', '')

            # # if int(img_num) > 5000:
            # #     os._exit(1)

            # save_path = os.path.join(save_root, img_num + '_lane_gt_vis.png')
            # lanes_gt_vis = lanes_gt / lanes_gt.max() * 255
            # cv2.imwrite(save_path, lanes_gt_vis.cpu().numpy())

            # save_path = os.path.join(save_root, img_num + '_crossing_gt_vis.png')
            # crossing_gt_vis = crossing_gt / crossing_gt.max() * 255
            # cv2.imwrite(save_path, crossing_gt_vis.cpu().numpy())

            # save_path = os.path.join(save_root, img_num + '_contour_gt_vis.png')
            # contour_gt_vis = contour_gt / contour_gt.max() * 255
            # cv2.imwrite(save_path, contour_gt_vis.cpu().numpy())

            # save_path = os.path.join(save_root, img_num + '_drivable_gt_vis.png')
            # drivable_gt_vis = drivable_gt / drivable_gt.max() * 255
            # cv2.imwrite(save_path, drivable_gt_vis.cpu().numpy())

            # save_path = os.path.join(save_root, img_num + '_lane_pred_vis.png')
            # lanes_pred_vis = lanes_pred / lanes_pred.max() * 255
            # cv2.imwrite(save_path, lanes_pred_vis.cpu().numpy())

            # save_path = os.path.join(save_root, img_num + '_drivable_pred_vis.png')
            # drivable_pred_vis = drivable_pred / drivable_pred.max() * 255
            # cv2.imwrite(save_path, drivable_pred_vis.cpu().numpy())
    ...
    def forward_train(self,
        ...
        """
        pred_seg_dict = self(bev_feat, img_metas=img_metas, veh2inf_rt=veh2inf_rt)
        loss_inputs = [
