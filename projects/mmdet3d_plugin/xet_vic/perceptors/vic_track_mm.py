...
class VICTrackMM(UniADTrack):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(VICTrackMM, self).__init__(
    ...
    def forward_train(self,
        ...
        if self.is_cooperation:
            ...
            pc = [points_veh]
        losses_track, outs_track = self.forward_track_train(pc, img, gt_bboxes_3d, gt_labels_3d, gt_past_traj, gt_past_traj_mask, gt_inds, gt_sdc_bbox, gt_sdc_label,
    ...
    def _forward_single_frame_train(
        ...
        bev_embed, bev_pos = self.get_bevs(
            pc, img, img_metas,
            prev_pc=prev_pc, prev_img=prev_img, prev_img_metas=prev_img_metas,
        )
    ...
    @auto_fp16(apply_to=("pc", "imgs", "prev_bev", "prev_img"))
    def get_bevs(self, pc, imgs, img_metas, prev_pc=None, prev_img=None, prev_img_metas=None, prev_bev=None):
        if prev_pc is not None and prev_img_metas is not None:
            assert prev_bev is None
            prev_bev = self.get_history_bev_pc(prev_pc, prev_img_metas)
    ...
    def get_history_bev_pc(self, pc_queue, img_metas_list):
        """
