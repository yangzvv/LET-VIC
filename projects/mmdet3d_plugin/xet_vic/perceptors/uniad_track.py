...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        if self.is_cooperation:
            self.cross_agent_query_interaction = CrossAgentSparseInteraction(pc_range=self.pc_range,
        ...
        # self.dropout_feat1 = nn.Dropout(0)
        # self.dropout_feat2 = nn.Dropout(0)
        ...
        # self.self_attn = nn.MultiheadAttention(embed_dims, 8, 0)
    ...
    def _forward_single_frame_train(
        ...
        if self.is_cooperation:
            ...
            track_instances = self.cross_agent_query_interaction(inf_track_query, track_instances, veh2inf_rt)
    ...
    def _forward_single_frame_inference(
        ...
        if self.is_cooperation:
            ...
            track_nums_src = len(track_instances)
            track_instances = self.cross_agent_query_interaction(inf_track_query, track_instances, veh2inf_rt, debug=False, name=img_metas[0]['filename'][0].split('/')[1])
            track_nums_new = len(track_instances)
            add_nums = track_nums_new - track_nums_src
