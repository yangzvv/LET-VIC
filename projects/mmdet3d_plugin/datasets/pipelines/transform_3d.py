...
from mmdet3d.datasets.pipelines.transforms_3d import ObjectRangeFilter, ObjectNameFilter, PointsRangeFilter
...
class XETVICCollect3D(object):
    ...
    def __init__(
        ...
        self.pc_meta_keys = pc_meta_keys
    ...
    def __call__(self, results):
        ...
        """
        data = {}
        dict_meta_key_pc2img = {
            'pc_filename': 'filename',
            'lidar2pseudoimg': 'lidar2img',
            'pseudoimg_shape': 'img_shape',
            'pseudoimg_ori_shape': 'img_ori_shape',
            'pseudoimg_pad_shape': 'pad_shape',
            'pseudoimg_scale_factor': 'scale_factor',
        }
        if self.use_camera:
            img_metas = {}
        ...
        if self.use_lidar:
            pc_metas = {}
            for key in self.pc_meta_keys:
                if key in results:
                    if key in dict_meta_key_pc2img.keys():
                        pc_metas[dict_meta_key_pc2img[key]] = results[key]
                    else:
                        pc_metas[key] = results[key]
            data['pc_metas'] = DC(pc_metas, cpu_only=True)
            if not self.use_camera:
                data['img_metas'] = data['pc_metas']
...
@PIPELINES.register_module()
class XETVICObjectRangeFilterTrack(object):
...
@PIPELINES.register_module()
class XETVICObjectNameFilterTrack(object):
...
class CustomObjectRangeFilter(ObjectRangeFilter):
    def __call__(self, results):
        ...
        """
        # Check points instance type and initialise bev_range
        if isinstance(results['gt_bboxes_3d'], (LiDARInstance3DBoxes, DepthInstance3DBoxes)):
            bev_range = self.pcd_range[[0, 1, 3, 4]]
...
@PIPELINES.register_module()
class XETVICPointsRangeFilter(PointsRangeFilter):
    """Filter points by the range.

    Args:
        point_cloud_range (list[float]): Point cloud range.
    """
    def __init__(self, point_cloud_range, point_cloud_range_inf):
        # super(XETVICPointsRangeFilter, self).__init__()
        self.pcd_range = np.array(point_cloud_range, dtype=np.float32)
        self.pcd_range_inf = np.array(point_cloud_range_inf, dtype=np.float32)
    ...
    def __call__(self, input_dict):
        ...
        """
        points_veh = input_dict['points_veh']
        points_veh_mask = points_veh.in_range_3d(self.pcd_range)
        clean_points_veh = points_veh[points_veh_mask]
        input_dict['points_veh'] = clean_points_veh
        points_veh_mask = points_veh_mask.numpy()
        pts_instance_mask = input_dict.get('pts_instance_mask', None)
        pts_semantic_mask = input_dict.get('pts_semantic_mask', None)
        if pts_instance_mask is not None:
            input_dict['pts_instance_mask'] = [pts_instance_mask[0][points_veh_mask]]

        if pts_semantic_mask is not None:
            input_dict['pts_semantic_mask'] = [pts_semantic_mask[0][points_veh_mask]]

        if 'points_inf' in input_dict:
            points_inf = input_dict['points_inf']
            points_inf_mask = points_inf.in_range_3d(self.pcd_range_inf)
            clean_points_inf = points_inf[points_inf_mask]
            input_dict['points_inf'] = clean_points_inf
            points_inf_mask = points_inf_mask.numpy()
            if pts_instance_mask is not None:
                input_dict['pts_instance_mask'].append(pts_instance_mask[1][points_inf_mask])

            if pts_semantic_mask is not None:
                input_dict['pts_semantic_mask'].append(pts_semantic_mask[1][points_inf_mask])
